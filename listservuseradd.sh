...
ONYEN_COUNTER=1
onyens_to_add=$*
if [[ $onyens_to_add == "" ]] ; then
	printf "\nNo onyen given\n"
	printf "\nUsage: listservuseradd.sh <onyen> <onyen> <onyen> ...\n\n"
	exit 1
fi
ONYENS_TO_ADD_COUNT=$(wc -w <<< $onyens_to_add)
email_template=""

#This helps with formatting
echo

for onyen in $onyens_to_add
do
	#If less than 6 onyens, don't display counter. Just look up the email and name associated with the onyen.
	if [[ ONYENS_TO_ADD_COUNT -lt 6 ]] ; then	
	...
	#If more than 6 onyens, display counter. Also look up the email and name associated with the onyen.
		...
		echo "Working on onyen $ONYEN_COUNTER/$ONYENS_TO_ADD_COUNT"
		onyen_email=$($UNC_LDAPSEARCH_CMD $onyen mail | grep mail | awk '{print $2}')
                onyen_name=$($UNC_LDAPSEARCH_CMD $onyen cn | grep cn | awk -F ': ' '{print $2}')
...
#Send the email to add all the users to the listserv
	printf "login $LISTSERV_ADMIN_PW\nadd lbg-users quiet <<\n$email_template>>" | mail -s "" -r "root@lbg-admin2.bioinf.unc.edu" lyris@listserv.unc.edu
	if [[ ONYENS_TO_ADD_COUNT -lt 6 ]] ; then
		printf "\n\033[A"
	else
		printf "\n\033[A\n\n"
	...
	echo
	printf "\n\033[ADone! To check the users were added correctly, go to https://lists.unc.edu\n\n"
fi
