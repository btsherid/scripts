...
if [[ $onyens_to_delete == "" ]] ; then
        printf "\nNo onyen given\n"
        printf "\nUsage: listservuserdelete.sh <onyen> <onyen> <onyen> ...\n\n"
        exit 1
fi
...
if [[ $email_template == "" ]] ; then
	...
	if [[ ONYENS_TO_DELETE_COUNT -lt 6 ]] ; then
        ...
        echo
	printf "\n\033[ADone! To check that the users were removed successfully, go to https://lists.unc.edu\n\n"
fi
