...
if [[ $onyens_to_add == "" ]] ; then
	printf "\nNo onyen given\n"
	printf "\nUsage: ide-listservuseradd.sh <onyen> <onyen> <onyen> ...\n\n"
	exit 1
fi
...
#Send the email to add all the users to the listserv
	...
	echo
	printf "\n\033[ADone! To check the users were added correctly, go to https://<listserv web interface URL>\n\n"
fi
